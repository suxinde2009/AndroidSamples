...
class ExtensionManager(private val mDispatcher: Dispatcher) {
    ...
    private val mExtensions = mutableMapOf<String, ExtensionInfo>()
    private val mExtensionLookup = mutableMapOf<Int, ExtensionInfo>()
    ...
    private fun addExtension(extension: Extension) {
        ...
        mExtensionLookup[info.mMajorOpCode.toInt()] = info
    ...
    fun queryExtension(name: String): ExtensionInfo {
        ...
        if (info == null) {
            Platform.logd(TAG, "Query for absent extension $name")
