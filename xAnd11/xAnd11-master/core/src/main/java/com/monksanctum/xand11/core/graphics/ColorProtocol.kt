...
class ColorProtocol(private val mGraphics: GraphicsManager) : Dispatcher.PacketHandler {
    ...
    override fun handleRequest(client: Client, reader: PacketReader, writer: PacketWriter) {
        when (reader.majorOpCode) {
            Request.QUERY_COLORS.code -> handleQueryColors(reader, writer)
            Request.ALLOC_COLOR.code -> handleAllocColor(reader, writer)
            Request.LOOKUP_COLOR.code -> handleLookupColor(reader, writer)
            Request.ALLOC_NAMED_COLOR.code -> handleAllocNamedColor(reader, writer)
        }
    }
    ...
    companion object {
        private val HANDLED_OPS = byteArrayOf(Request.QUERY_COLORS.code,
                Request.ALLOC_COLOR.code,
                Request.LOOKUP_COLOR.code,
                Request.ALLOC_NAMED_COLOR.code)
    }
}
