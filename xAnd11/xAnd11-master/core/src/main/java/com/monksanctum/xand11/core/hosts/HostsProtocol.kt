...
class HostsProtocol(private val mHostManager: HostsManager) : Dispatcher.PacketHandler {
    ...
    override fun handleRequest(client: Client, reader: PacketReader, writer: PacketWriter) {
        when (reader.majorOpCode) {
            Request.LIST_HOSTS.code -> handleListHosts(reader, writer)
            Request.CHANGE_HOSTS.code -> handleChangeHosts(reader, writer)
        }
    }

    companion object {

        private val HANDLED_OPS = byteArrayOf(Request.LIST_HOSTS.code,
                Request.CHANGE_HOSTS.code)
        ...
        fun writeHost(host: Host, writer: XProtoWriter) {
            ...
            writer.writePaddedString(String(host.address!!.map {
                it.toChar()
            }.toCharArray()))
        }
        ...
        fun readHost(reader: XProtoReader): Host {
            ...
            host.address = bytes.map { it.toByte() }.toByteArray()
            return host
        }
    }
}
