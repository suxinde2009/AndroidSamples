...
import org.monksanctum.xand11.core.printStackTrace
...
class XWindowProtocol(private val mWindowManager: XWindowManager) : Dispatcher.PacketHandler {
    ...
    override fun handleRequest(client: Client, reader: PacketReader, writer: PacketWriter) {
        when (reader.majorOpCode) {
            Request.CREATE_WINDOW.code -> handleCreateWindow(client, reader)
            Request.CHANGE_WINDOW_ATTRIBUTES.code -> handleChangeWindowAttributes(client, reader)
            Request.CHANGE_PROPERTY.code -> handleChangeProperty(reader)
            Request.GET_PROPERTY.code -> handleGetProperty(reader, writer)
            Request.DELETE_PROPERTY.code -> handleDeleteProperty(reader)
            Request.MAP_WINDOW.code -> handleMapWindows(reader)
            Request.MAP_SUBWINDOWS.code -> handleMapSubwindows(reader)
            Request.UNMAP_WINDOW.code -> handleUnmapWindows(reader)
            Request.UNMAP_SUBWINDOWS.code -> handleUnmapSubwindows(reader)
            Request.DESTROY_WINDOW.code -> handleDestroyWindows(reader)
            Request.DESTROY_SUBWINDOWS.code -> handleDestroySubwindows(reader)
            Request.CONFIGURE_WINDOW.code -> handleConfigureWindow(reader)
            Request.GET_WINDOW_ATTRIBUTES.code -> handleGetWindowAttributes(client, reader, writer)
            Request.CLEAR_AREA.code -> handleClearArea(reader)
            Request.QUERY_TREE.code -> handleQueryTree(reader, writer)
            Request.REPARENT_WINDOW.code -> handleReparentWindow(reader)
        }
    }
    ...
    private fun handleChangeProperty(reader: PacketReader) {
        ...
        property?.let {
            ...
            it.change(mode, value.map {it.toByte()}.toByteArray())
        }
        }
        synchronized(window) {
    ...
    private fun handleGetProperty(reader: PacketReader, writer: PacketWriter) {
        ...
        synchronized(property) {
            ...
            writer.writePaddedString(String(property.value.map {
                it.toChar()
            }.toCharArray()))
        }
    }
    ...
    companion object {
        ...
        private val HANDLED_OP_CODES = byteArrayOf(Request.GET_PROPERTY.code,
                Request.DELETE_PROPERTY.code,
                Request.CHANGE_PROPERTY.code,
                Request.CREATE_WINDOW.code,
                Request.CHANGE_WINDOW_ATTRIBUTES.code,
                Request.MAP_SUBWINDOWS.code,
                Request.MAP_WINDOW.code,
                Request.CONFIGURE_WINDOW.code,
                Request.GET_WINDOW_ATTRIBUTES.code,
                Request.CLEAR_AREA.code,
                Request.UNMAP_SUBWINDOWS.code,
                Request.UNMAP_WINDOW.code,
                Request.DESTROY_SUBWINDOWS.code,
                Request.DESTROY_WINDOW.code,
                Request.QUERY_TREE.code,
                Request.REPARENT_WINDOW.code)
    }
}
