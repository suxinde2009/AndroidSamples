...
import org.monksanctum.xand11.core.IntObjectPool
 ...
 * the reader.
 */
class PacketReader(val majorOpCode: Byte, val minorOpCode: Byte) : XProtoReader(null) {
    ...
    class ByteHolder(size: Int) : ObjectPool.Recycleable() {
    ...
    companion object {
        private val sArrays: IntObjectPool<ByteHolder> =
            object : IntObjectPool<ByteHolder>() {
                protected override fun validate(inst: ByteHolder, vararg arg: Int): Boolean {
                    val length = inst.mHolder.size
                    return if (length > arg[0] * 4) {
                        false // Save really big ones for big packets.
                    } else length >= arg[0]
                }

                protected override fun create(vararg arg: Int): ByteHolder {
                    return ByteHolder(arg[0])
                }
            }
    }
}
