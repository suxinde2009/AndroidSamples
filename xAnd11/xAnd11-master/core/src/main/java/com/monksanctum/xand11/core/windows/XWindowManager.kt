...
class XWindowManager(private val mInfo: XServerInfo, internal val graphicsManager: GraphicsManager,
    ...
    private val mWindows = mutableMapOf<Int, XWindow>()
    ...
    fun getWindow(windowId: Int): XWindow {
        //if (DEBUG) Platform.logd(TAG, "getWindow " + Integer.toHexString(windowId));
        val window = mWindows.get(windowId)
        if (window == null) {
            Platform.logw(TAG, "Attempt to get unknown window $windowId")
    ...
    companion object {
        ...
        internal val DEBUG = WINDOW_DEBUG
